...
void print_arrayZiptable(ziptable_t ziptable[], int size){
...
int main(void) {
    ...
    for(int i=0; i < NUM; i++){
        n = init(ziptable);
        start = clock();
        qsort(ziptable, n, sizeof(ziptable[0]), compare_ziptable);
        end = clock();
        sum += end - start;
    }

    print_arrayZiptable(ziptable, n);
